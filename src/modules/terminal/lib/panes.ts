import type { TerminalPaneColorId } from "./paneColors";
...
export function setLeafColor(
  n: PaneNode,
  id: PaneId,
  color: TerminalPaneColorId | undefined,
): PaneNode {
  if (isLeaf(n)) {
    if (n.id !== id || n.color === color) return n;
    if (color === undefined) {
      const next = { ...n };
      delete next.color;
      return next;
  ...
  let changed = false;
  const next = n.children.map((c) => {
    const u = setLeafColor(c, id, color);
    if (u !== c) changed = true;
    return u;
  });
  return changed ? { ...n, children: next } : n;
